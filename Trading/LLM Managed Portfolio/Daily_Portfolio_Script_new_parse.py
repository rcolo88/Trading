...
class DailyPortfolioReport:
    ...
    def parse_text_content(self, content: str):
        ...
        # Enhanced parsing with context awareness for LLM-generated content
        orders.extend(self._parse_with_context_awareness(orders_section))
        
        # Fallback to line-by-line parsing if no orders found
        if not orders:
            print("🔄 Using fallback line-by-line parsing...")
            orders.extend(self._parse_line_by_line(orders_section))
        
        # Final validation and deduplication
        orders = self._deduplicate_orders(orders)
        
        if not orders:
            print("❌ No valid trading orders found after enhanced parsing")
            print("💡 Consider checking document format or adding debug output")
        
        return orders
    
    def _parse_with_context_awareness(self, content: str):
        """Context-aware parsing for complex LLM-generated trading recommendations"""
        orders = []
        
        # Enhanced patterns that can handle complex sentence structures
        complex_patterns = [
            # "IONS FDA decision Aug 21 creates unacceptable portfolio risk. Sell 2 shares at market"
            r'(?P<context>[^.]+[Ss]ell[^.]*?)\s*[.-]?\s*[Ss]ell:?\s*(?P<shares>\d+)\s+shares?\s+(?:at\s+market|of\s+(?P<ticker>[A-Z]{2,5}))?',
            
            # "Reduce concentration from 21 to 8 shares" or "Reduce exposure by 25%"
            r'(?P<context>[^.]+)[Rr]educe\s+(?:concentration|exposure)\s+(?:from\s+\d+\s+to\s+(?P<shares>\d+)|by\s+(?P<shares2>\d+))\s+shares?.*?(?P<ticker>[A-Z]{2,5})',
            
            # "Add 1 share over next 30 days on weakness to $190-195 support"
            r'(?P<context>[^.]+)[Aa]dd\s+(?P<shares>\d+)\s+shares?\s+(?:over|within|during).*?(?P<ticker>[A-Z]{2,5})',
            
            # "Hold current position through earnings" or "Hold position"
            r'(?P<context>[^.]+)[Hh]old\s+(?:current\s+)?position.*?(?P<ticker>[A-Z]{2,5})',
            
            # "Complete exit at $2.96" with ticker nearby
            r'(?P<context>[^.]+)[Cc]omplete\s+exit.*?(?P<ticker>[A-Z]{2,5})',
            
            # "Generate $103 cash for redeployment" - usually indicates selling all
            r'(?P<context>[^.]+)[Gg]enerate\s+\$\d+\s+cash.*?(?P<ticker>[A-Z]{2,5})',
        ]
        
        for pattern in complex_patterns:
            matches = re.finditer(pattern, content, re.IGNORECASE | re.DOTALL)
            for match in matches:
                order = self._parse_complex_match(match)
                if order:
                    orders.append(order)
        
        return orders
    
    def _parse_complex_match(self, match):
        """Parse a complex regex match into a trading order"""
        groups = match.groupdict()
        context = groups.get('context', '').strip()
        ticker = groups.get('ticker')
        shares = groups.get('shares') or groups.get('shares2')
        
        if not ticker:
            # Try to find ticker in the context
            ticker_match = re.search(r'\b([A-Z]{2,5})\b', context)
            if ticker_match:
                ticker = ticker_match.group(1)
            else:
                return None
        
        # Determine action based on context keywords
        context_lower = context.lower()
        
        if any(keyword in context_lower for keyword in ['sell', 'exit', 'liquidate', 'reduce']):
            action = OrderType.SELL
            if not shares:
                # Look for "all shares" or "entire position" indicators
                if any(phrase in context_lower for phrase in ['all shares', 'entire position', 'complete exit']):
                    shares = None  # Will be handled as "all available shares"
                else:
                    return None
        elif any(keyword in context_lower for keyword in ['buy', 'add', 'acquire', 'initiate']):
            action = OrderType.BUY
            if not shares:
                return None
        elif any(keyword in context_lower for keyword in ['hold', 'maintain', 'keep']):
        ...
        if any(keyword in context_lower for keyword in ['urgent', 'immediate', 'critical', 'binary catalyst']):
            priority = OrderPriority.HIGH
        elif any(keyword in context_lower for keyword in ['strategic', 'long-term', 'gradual']):
        ...
        return TradeOrder(
            ...
            reason=context[:200] if context else "Extracted from complex sentence",
            priority=priority
        )
    
    def _parse_line_by_line(self, content: str):
    ...
    def _deduplicate_orders(self, orders):
        """Remove duplicate orders based on ticker and action"""
        seen = set()
        deduplicated = []
        
        for order in orders:
            key = (order.ticker, order.action)
            if key not in seen:
                seen.add(key)
                deduplicated.append(order)
            else:
                print(f"🔄 Skipping duplicate order: {order.action.value} {order.ticker}")
        
        return deduplicated
    
    def _extract_orders_section(self, content: str):
        """Extract the ORDERS section from markdown with enhanced LLM-generated content detection"""
        # Enhanced patterns to capture various LLM-generated section headers
        order_section_patterns = [
            # Traditional ORDERS section
            r'##\s+ORDERS\s*\n(.*?)(?=\n##\s+(?:MARKET ANALYSIS|STRATEGIC ALLOCATION|EXECUTION NOTES|RISK MANAGEMENT)|\Z)',
            # LLM common patterns
            r'##\s+(?:IMMEDIATE\s+EXECUTION|TRADING\s+ORDERS|RECOMMENDED\s+TRADES).*?\n(.*?)(?=\n##|\Z)',
            r'##\s+(?:POSITION\s+MANAGEMENT|TRADE\s+EXECUTION).*?\n(.*?)(?=\n##|\Z)',
            # Section with priority keywords
            r'(.*?(?:SELL|BUY|HOLD).*?(?:shares?|position).*?)(?=\n##\s+(?:MARKET|STRATEGIC|EXECUTION|RISK)|\Z)',
        ]
        
        for pattern in order_section_patterns:
    ...
    def _parse_order_line(self, line: str, priority: OrderPriority):
        """Enhanced LLM-aware parsing for natural language trading instructions"""
        # Clean the line of markdown and common formatting
        original_line = line
        ...
        # Skip common LLM section headers
        skip_patterns = [
            r'^#{1,6}\s+',  # Markdown headers
            r'^(action|recommendation|strategy|rationale):?\s*$',  # Common LLM headers
        ...
        # Extract ticker symbol first (more flexible matching)
        ticker_match = re.search(r'\b([A-Z]{2,5})\b', line)
        if not ticker_match:
            return None
        ticker = ticker_match.group(1)
        
        # Enhanced SELL patterns for LLM natural language
        sell_patterns = [
            # Traditional patterns
            r'sell\s+(?:all\s+)?(\d+)\s+shares?\s+(?:of\s+)?\b' + re.escape(ticker) + r'\b',
            # Natural language patterns from LLMs
            r'\b' + re.escape(ticker) + r'\b.*?sell\s+(\d+)\s+shares?',
            r'reduce\s+(?:position\s+in\s+)?\b' + re.escape(ticker) + r'\b.*?(\d+)\s+shares?',
            r'exit\s+(?:all\s+)?(\d+)\s+shares?\s+(?:of\s+)?\b' + re.escape(ticker) + r'\b',
            r'liquidate\s+(\d+)\s+shares?\s+(?:of\s+)?\b' + re.escape(ticker) + r'\b',
            # "Sell: X shares at market" pattern from august 19 recommendations
            r'sell:?\s+(\d+)\s+shares?\s+(?:at\s+market|of\s+' + re.escape(ticker) + r')?',
            # "Reduce exposure by X shares" pattern
            r'reduce\s+(?:exposure\s+)?(?:by\s+)?(\d+)\s+shares?',
        ]
        ...
        for pattern in sell_patterns:
            ...
            if match:
                ...
                reason = self._extract_reason(original_line)
                return TradeOrder(
                    ticker=ticker,
                    action=OrderType.SELL,
                    shares=shares,
        ...
        # Enhanced BUY patterns for LLM natural language
        buy_patterns = [
            # Traditional patterns
            r'buy\s+(\d+)\s+shares?\s+(?:of\s+)?\b' + re.escape(ticker) + r'\b',
            # Natural language patterns from LLMs
            r'\b' + re.escape(ticker) + r'\b.*?(?:buy|add|purchase)\s+(\d+)\s+shares?',
            r'(?:add|acquire|purchase)\s+(\d+)\s+shares?\s+(?:of\s+)?\b' + re.escape(ticker) + r'\b',
            r'initiate\s+(?:position\s+)?(?:in\s+)?\b' + re.escape(ticker) + r'\b.*?(\d+)\s+shares?',
            r'build\s+(?:position\s+)?(?:in\s+)?\b' + re.escape(ticker) + r'\b.*?(\d+)\s+shares?',
            # "Add X share over next 30 days" pattern
            r'add\s+(\d+)\s+shares?\s+(?:over|within|during)',
        ]
        ...
        # Enhanced HOLD patterns for LLM natural language
        hold_patterns = [
            # Traditional patterns
            r'hold\s+(?:all\s+)?(\d+)?\s*shares?\s+(?:of\s+)?\b' + re.escape(ticker) + r'\b',
            # Natural language patterns from LLMs
            r'\b' + re.escape(ticker) + r'\b.*?hold\s+(?:current\s+)?(?:position|shares?)',
            r'maintain\s+(?:position\s+)?(?:in\s+)?\b' + re.escape(ticker) + r'\b',
            r'keep\s+(?:current\s+)?(?:position\s+)?(?:in\s+)?\b' + re.escape(ticker) + r'\b',
            # "Hold current position" pattern
            r'hold\s+current\s+position',
            r'maintain\s+(?:current\s+)?exposure',
        ]
        
        for pattern in hold_patterns:
            match = re.search(pattern, line, re.IGNORECASE)
            if match:
                shares = int(match.group(1)) if match.groups() and match.group(1) else None
                return TradeOrder(
                    ticker=ticker,
                    action=OrderType.HOLD,
                    shares=shares,
        ...
        # If line contains a ticker but no clear action, log for debugging
        print(f"⚠️ Ticker {ticker} found but no clear action in: '{line[:100]}...'")
        return None
    
    def _extract_reason(self, line: str):
        """Enhanced reason extraction for LLM-generated content"""
        ...
        for pattern in reason_patterns:
            ...
            if match:
                ...
                reason = re.sub(r'^(of|to|the)\s+', '', reason, flags=re.IGNORECASE)
                if len(reason) > 10:  # Only return substantial reasons
                    return reason
        
        # Extract everything after ticker and action as fallback
        # Remove common action words and ticker symbols
        cleaned = re.sub(r'\b(SELL|BUY|HOLD|ADD|REDUCE|EXIT)\b', '', line, flags=re.IGNORECASE)
        cleaned = re.sub(r'\b[A-Z]{2,5}\b', '', cleaned)  # Remove tickers
        cleaned = re.sub(r'\d+\s+shares?', '', cleaned, flags=re.IGNORECASE)  # Remove share counts
        cleaned = re.sub(r'^[\s\-•]+', '', cleaned)  # Remove leading punctuation
        cleaned = cleaned.strip()
        
        # Return a meaningful portion if found
        if len(cleaned) > 15:  # Minimum length for meaningful reason
...
def test_enhanced_parser():
    """Test the enhanced parser with sample LLM-generated content"""
    print("\n" + "="*60)
    print("🔬 TESTING ENHANCED LLM-AWARE PARSER")
    print("="*60)
    
    # Sample text from the August 19 recommendations that failed to parse
    sample_content = """
...
"""
    ...
    print("\n🔍 Testing enhanced parsing on sample content...")
    orders = portfolio.parse_text_content(sample_content)
    ...
    if orders:
        ...
        for i, order in enumerate(orders, 1):
            shares_text = f"{order.shares} shares" if order.shares else "current position"
            print(f"   {i}. {order.action.value} {shares_text} of {order.ticker} ({order.priority.value})")
            print(f"      Reason: {order.reason[:100]}...")
    else:
        print("\n❌ NO ORDERS PARSED - Parser needs further enhancement")
...
# USAGE:
# For testing the enhanced parser:
# Run: python Daily_Portfolio_Script_new_parse.py --test-parser
#
# For trading execution:
# 1. Save Claude's response as 'claude_recommendations.txt'
# 2. Run: python Daily_Portfolio_Script_new_parse.py
# 3. Review execution report and update holdings in script
#
# For read-only portfolio report (no trades executed):
# Run: python Daily_Portfolio_Script_new_parse.py --report-only
