    ...
    def sync_to_local_portfolio(self, account_data: Dict = None) -> bool:
        ...
        try:
            ...
            for ticker in sorted(all_tickers):
                # Handle both dict and int formats for holdings
                local_holding = self.portfolio.holdings.get(ticker, 0)
                if isinstance(local_holding, dict):
                    local_shares = local_holding.get('shares', 0)
                else:
                    local_shares = local_holding
