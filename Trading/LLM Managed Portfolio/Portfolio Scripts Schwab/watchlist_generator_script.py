...
class WatchlistGenerator:
    ...
    def export_results(self, ranked_stocks: List[tuple], all_quality_results: Dict[str, QualityAnalysisResult]):
        ...
        # Export CSV (primary watchlist)
        csv_file = output_dir / f"quality_watchlist_{timestamp}.csv"
        with open(csv_file, 'w', newline='') as f:
            writer = csv.writer(f)
            writer.writerow(['Ticker', 'Quality_Score', 'Tier', 'Red_Flags', 'Gross_Profitability', 'ROE', 'ROIC', 'FCF_Yield', 'Operating_Profitability'])
        ...
        # Export full JSON
        json_file = output_dir / f"quality_watchlist_{timestamp}_full.json"
        json_data = {
        ...
        # Export summary text
        summary_file = output_dir / f"quality_watchlist_{timestamp}_summary.txt"
        with open(summary_file, 'w') as f:
            ...
            f.write("-"*60 + "\n")
            f.write(f"{'Rank':<6} {'Ticker':<8} {'Score':<8} {'Tier':<12} {'Red Flags':<10}\n")
            f.write("-"*60 + "\n")

            for rank, (ticker, result) in enumerate(ranked_stocks[:50], 1):
                f.write(f"{rank:<6} {ticker:<8} {result.composite_score:6.1f}   {result.tier.value:<12} {len(result.red_flags):<10}\n")
            ...
            if elite_stocks:
                for ticker, result in elite_stocks:
                    f.write(f"\n{ticker}: {result.composite_score:.1f}\n")
                    f.write(f"  Metrics:\n")
