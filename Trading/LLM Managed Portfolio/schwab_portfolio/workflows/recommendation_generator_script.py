...
class RecommendationGenerator:
    ...
    def load_latest_analysis(self, analysis_type: str) -> Optional[Dict]:
        """
        Load analysis file (news or quality) - fixed filenames, no timestamps
        ...
        # Fixed filenames - no timestamp patterns
        filename = f"{analysis_type}_analysis.json"
        filepath = output_dir / filename

        if not filepath.exists():
            logger.warning(f"No {analysis_type} analysis file found at {filepath}")
            return None

        logger.info(f"Loading {analysis_type} analysis from {filepath}")
    ...
    def generate_recommendations(self, market_analysis: Dict) -> List[Dict]:
        ...
        for ticker in holdings:
            ...
            agent_outputs = {
                ...
                'current_shares': self.portfolio_state['holdings'][ticker]
            }
    ...
    def export_trading_document(self, recommendations: List[Dict], market_analysis: Dict):
        ...
        with open(output_file, 'w') as f:
            ...
            if self.quality_analysis:
                holdings_quality = self.quality_analysis.get('holdings_quality', {})
                quality_scores = [d.get('composite_score', 0) for d in holdings_quality.values()]
                if quality_scores:
