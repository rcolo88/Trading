#!/usr/bin/env python3
"""
Bull Put Spread Parameter Optimization -- LOW-DELTA / WIDE-EXIT overnight run (2026-08-09/10)

Variant of optimize_bull_put_spread.py with widened ranges, built to chase down a specific
suspicion: a quick 0.12/0.15 delta probe on the full 2008-2026 span found 0.12Delta/0.08Delta as
the best structure in the whole project, with 0.15Delta a total failure right next to it -- then
an era-split showed 0.12Delta's edge was ~23x concentrated in 2008-2017 (no real options data
exists anywhere in this project before 2021), and a same-day/same-window (2018-2026) partial
re-check already found 0.12Delta/0.08Delta NEGATIVE (Sharpe -0.56, OOS -0.09) at the original
exit params. This run uses ONLY the 2018-2026 dataset (config.yaml's current window -- the more
reliable one) and searches short_delta/long_delta/profit_target/stop_loss/dte_min jointly via
Optuna, walk-forward (IS 2018-23, OOS 2023-26) as the default, to answer honestly whether ANY
point in the low-delta region survives on real(er) data, or whether 0.15Delta's cliff and
0.12Delta's collapse generalize.

Sized for an ~8-hour unattended overnight run: N_TRIALS=650 at an empirically-measured ~40s/trial
on the IS window ~= 7.2 hours, leaving margin for OOS scoring / DSR / reporting.
...
def setup_optimizer(config: Dict[str, Any], options_data: pd.DataFrame, underlying_data: pd.DataFrame,
    ...
    # 2026-08-09/10 low-delta/wide-exit ranges. Conventions that MATTER here:
    ...
    # short_delta/long_delta widened past both the original 16-30 tastytrade-anchored range AND the
    # suspicious 0.12/0.15 probe, to properly resolve the cliff between them rather than spot-check
    # it -- 0.01 step lets Optuna finely resolve that region since it samples, not enumerates.
    optimizer.set_parameter_range('dte', min=30, max=45, step=5)                  # unchanged: target 30-45 DTE
    optimizer.set_parameter_range('short_delta', min=0.10, max=0.32, step=0.01)   # was 0.16-0.30; now covers the 0.12/0.15 cliff + past 0.30
    optimizer.set_parameter_range('long_delta', min=0.05, max=0.15, step=0.01)    # was 0.08-0.16; centers the 0.08-vs-0.10 tradeoff found earlier
    optimizer.set_parameter_range('profit_target', min=0.20, max=0.90, step=0.10) # was 0.40-0.60; user asked for a much wider sweep
    optimizer.set_parameter_range('stop_loss', min=0.10, max=0.90, step=0.10)     # was 0.30-0.90 step 0.20; user asked for 0.10-0.80+, finer step
    optimizer.set_parameter_range('dte_min', min=5, max=23, step=2)               # was 18-24; user asked for "min dte" down to ~5
    optimizer.set_parameter_range('vix_min', min=10, max=25, step=5)              # unchanged: 10≈off .. 25=rich IV
...
def run_optimization(optimizer: ParameterOptimizer) -> pd.DataFrame:
    ...
    if total_combinations > 1000:
        MODE = 'optuna'
        # 2026-08-09/10: sized for an unattended ~8h overnight run. Empirically measured ~40s/trial
        # on the IS window (2018-01-02..2023-12-20) via optimizer._run_single_backtest directly --
        # 650 * 40s ~= 7.2h, leaving ~50min margin for data load / final OOS scoring / DSR / I/O.
        N_TRIALS = 650
    else:
